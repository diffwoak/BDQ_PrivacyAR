...
# 在初始化时发出警告，提示这些类将在未来版本中被移除，并建议使用 torch.nn 中的对应类
...
# 一个冻结的批量归一化层，其中批量统计和仿射参数是固定的。
# 这个类在初始化时会注册权重、偏置、运行均值和运行方差作为缓冲区，并提供了从状态字典加载参数的方法
# This is not in nn
...
# 一个配置化的块，用于卷积-归一化-激活块。
# 它允许自定义卷积层、归一化层和激活层的组合，并提供了灵活的参数设置。
...
# 一种注意力机制，用于增强特定通道的特征响应。它包含平均池化、两个全连接层以及激活函数
