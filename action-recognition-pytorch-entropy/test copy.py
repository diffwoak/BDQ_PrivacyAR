...
def main():
    ...
    model_target.eval()
    model_budget = model_budget.cuda()
    model_budget.eval()

    args.resume = 'results/sbu/adv/"

    print(args.resume)

    print("=> using pre-trained model '{}'".format(arch_name))
    checkpoint = torch.load(args.resume+'model_degrad_e50.ckpt', map_location='cpu')
    model_degrad.load_state_dict(checkpoint['state_dict'])
    del checkpoint
    checkpoint = torch.load(args.resume+'model_target_e50.ckpt', map_location='cpu')
    model_target.load_state_dict(checkpoint['state_dict'])
    del checkpoint
    checkpoint = torch.load(args.resume+'model_budget_e50.ckpt', map_location='cpu')
    ...
    with torch.no_grad(), tqdm(total=total_batches) as t_bar:
        ...
        # for i, (video, target_actor, target_action) in enumerate(data_loader):
            output = eval_a_batch(video, model, num_clips=args.num_clips, num_crops=args.num_crops, threed_data=args.threed_data)
            if args.evaluate:
