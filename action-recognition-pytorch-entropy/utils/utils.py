...
def get_augmentor(is_train, image_size, mean=None,
    ...
    if is_train:
        if version == 'v1':
            augments += [
                GroupMultiScaleCrop(image_size, [1, .875, .75, .66])# 图像在多个尺度上进行裁剪
            ]
        elif version == 'v2':
            augments += [
                GroupRandomScale(scale_range),
                GroupRandomCrop(image_size), # 随机缩放和裁剪
            ]
        augments += [GroupRandomHorizontalFlip(is_flow=is_flow)] # 随机水平翻转图像
    else:
        scaled_size = image_size if disable_scaleup else int(image_size / 0.875 + 0.5)
        augments += [
            GroupScale(scaled_size),
            GroupCenterCrop(image_size)# 缩放和裁剪图像
        ]
    augments += [
        Stack(threed_data=threed_data),
        ToTorchFormatTensor(),
        GroupNormalize(mean=mean, std=std, threed_data=threed_data) # 堆叠图像、转换为PyTorch张量格式，并进行归一化
    ]
...
def train(data_loader, model_degrad, model_target, model_budget, optimizer_t, optimizer_b, train_criterion, train_entropy_criterion, epoch, steps_per_epoch=99999999999, gpu_id=None, rank=0, weight = 2):
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            loss_t = loss_target - weight *entropy_budget + 10.
            loss_t.backward()
            optimizer_t.step()

            # 在每个 epoch 结束后清理 GPU 内存
    ...
    return top1_target.avg, top5_target.avg, top1_budget.avg, top5_budget.avg, losses.avg
...
def train1(data_loader, model_degrad, model_target, model_budget, optimizer, train_criterion, train_entropy_criterion, epoch, step, steps_per_epoch=99999999999,gpu_id=None, rank=0):
...
def validate1(data_loader, model_degrad, model_target, model_budget, train_criterion, step, gpu_id=None):
...
def train_target(data_loader, model_target, optimizer, train_criterion, epoch, gpu_id=None):
    losses_target= AverageMeter()
    top1_target= AverageMeter()
    top5_target= AverageMeter()
    # set different random see every epoch
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            # print(f"images:{images.size()},gpu_id:{gpu_id}")
            output_target = model_target(images)
            loss_target = train_criterion(output_target, target_action)
            ...
            top5_target.update(prec5_target[0], images.size(0))
            t_bar.update(1)
    return top1_target.avg, top5_target.avg, losses_target.avg

def validate_target(data_loader, model_target, train_criterion, gpu_id=None):
    losses_target= AverageMeter()
    losses= AverageMeter()
    top1_target= AverageMeter()
    top5_target= AverageMeter()
...
def train_budget(data_loader, model_budget, optimizer, train_criterion, epoch, gpu_id=None):
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            r = random.randint(0,images.size(2)-1)
            output_budget = model_budget(images[:,:,r,:,:]) 
            ...
            top5_budget.update(prec5_budget[0], images.size(0))
            t_bar.update(1)
    return top1_budget.avg, top5_budget.avg, losses_budget.avg

def validate_budget(data_loader, model_budget, train_criterion, gpu_id=None):
    losses_budget= AverageMeter()
    top1_budget= AverageMeter()
    top5_budget= AverageMeter()
        ...
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                for r in range(images.size(2)):
                    output_budget.append(model_budget(images[:,:,r,:,:])) 
               
            loss_budget = 0
            for r in range(images.size(2)):            
                loss_budget += train_criterion(output_budget[r], target_actor) 
            loss_budget /= images.size(2) 
            ...
            for r in range(images.size(2)):
                ...
                prec1_budget += prec1_budget_
                prec5_budget += prec5_budget_
            prec1_budget /= images.size(2) 
            prec5_budget /= images.size(2) 
            losses_budget.update(loss_budget.item(), images.size(0))
            top1_budget.update(prec1_budget[0], images.size(0))
            top5_budget.update(prec5_budget[0], images.size(0))
            t_bar.update(1)
