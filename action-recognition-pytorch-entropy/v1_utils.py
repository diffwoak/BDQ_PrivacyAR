...
def accuracy(output, target, topk=(1, 2)):
    ...
    with torch.no_grad():
        ...
        _, pred = output.topk(maxk, 1, True, True) # logits 里 最大的 maxk个值 以及 下标
        pred = pred.t()
...
def train(data_loader, model_degrad, model_target, model_budget, optimizer_t, optimizer_b, train_criterion, train_entropy_criterion, extra_loss, epoch, step, display=100, steps_per_epoch=99999999999,  
    clip_gradient=None,  gpu_id=None, rank=0, alp=2):
    alhpa = alp
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            images = images.cuda(gpu_id, non_blocking=True)  # ([2, 48, 224, 224])
            target_action = target_action.cuda(gpu_id, non_blocking=True) # [B]  每个是 label
            target_actor = target_actor.cuda(gpu_id, non_blocking=True)   # [B]  每个是 label
            ...
            output_degrad, bias = model_degrad(images) # ([2, 3, 15, 224, 224]), [15]
            output_target = model_target(output_degrad) #  ([Bs, cls])  B, cls 8分类
            r = random.randint(0,output_degrad.size(2)-1)
            output_budget = model_budget(output_degrad[:,:,r,:,:])  # 随机取一帧来做 隐私预测  torch.Size([Bs, 13])
            ...
            loss_t = loss_target - alhpa * entropy_budget + 1000.    # 最小化目标任务，最大化隐私开销
            loss_t.backward()
            optimizer_t.step()
            ...
            output_degrad, bias = model_degrad(images) # ([2, 3, 15, 224, 224])
            ...
            losses_target.update(loss_target.item(), images.size(0)) # batch 平均 loss
...
def train1(data_loader, model_degrad, model_target1, model_budget1, optimizer, train_criterion, train_entropy_criterion, epoch, step, d, display=100, steps_per_epoch=99999999999,  
    clip_gradient=None,  gpu_id=None, rank=0, anony=True):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with tqdm(total=len(data_loader)) as t_bar:
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            # --------no anony-------
            if anony == False:
                B, C_T, H, W= images.shape
                # from pdb import set_trace
                # set_trace()
                num_image = C_T // 3 
                images = images.reshape(B, num_image, 3, H, W)
                images= torch.transpose(images,1,2)
                output_target = model_target1(images)
                r = random.randint(0,images.size(2)-1)
                output_budget = model_budget1(images[:,:,r,:,:]) 
            else:
                with torch.no_grad():
                    output_degrad, bias = model_degrad(images)
...
def validate1(data_loader, model_degrad, model_target1, model_budget1, train_criterion, step,d, gpu_id=None,anony=True):
    batch_time = AverageMeter()
    data_time = AverageMeter()
        ...
        for i, (images, target_actor, target_action) in enumerate(data_loader):
            ...
            if anony == False:
                ...
                for r in range(images.size(2)):
                    output_budget.append(model_budget1(images[:,:,r,:,:])) 
                loss_target = train_criterion(output_target, target_action)
                loss_budget = 0
                for r in range(images.size(2)):            
                    loss_budget += train_criterion(output_budget[r], target_actor) 
