...
from .v1_utils import (train, validate, train1, validate1, build_dataflow, get_augmentor,
                         save_checkpoint)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model_degrad = model[0]  # anonymizer
    model_target = model[1]  # i3d-50
    model_budget = model[2]  # resnet50
    model_target1 = model[3] # i3d-50
    model_budget1 = model[4] # resnet50
    ...
    if args.show_model:
        if args.rank == 0:
            print(f'{model_degrad=}')
            print(f'{model_target=}')
            print(f'{model_budget=}')
            print(f'{model_target1=}')
            print(f'{model_budget1=}')
        return 0
    ...
    train_smoothl1_criterion = nn.SmoothL1Loss(reduction='mean').cuda(args.gpu)
    ...
    val_dataset = VideoDataSet(args.datadir, val_list, args.groups, args.frames_per_group, # 采样 groups=16 组 图片，每组 frames_per_group=1 帧
                               num_clips=args.num_clips,
                               modality=args.modality, image_tmpl=image_tmpl,  # image_tmpl 图片格式前缀
                               dense_sampling=args.dense_sampling,
                               transform=val_augmentor, is_train=False, test_mode=False,  # test模式不加载label
    ...
    sys.stdout.flush()
    # kinetics_path = '
    # kinetics_path = "../i3d_imagenet-pretrained-r50_8xb8-32x2x1-100e_kinetics400-rgb_20220812-e213c223.pth"
    kinetics_path = "./resnet50-19c8e357.pth"
    state_dict = torch.load(kinetics_path)
    new_model_state_dict = model_target.state_dict()
    state_d = inflate_from_2d_model(state_dict, new_model_state_dict,skipped_keys=['fc'])
    model_target.load_state_dict(state_d, strict=False)
    model_target = model_target.cuda(args.gpu)

    state_dict = torch.load(kinetics_path)
    new_model_state_dict = model_target1.state_dict()
    state_d = inflate_from_2d_model(state_dict, new_model_state_dict, skipped_keys=['fc'])
    model_target1.load_state_dict(state_d, strict=False)
    model_target1 = model_target1.cuda(args.gpu)
    # print("Loaded keys:", checkpoint['state_dict'].keys())
    # print("Model keys:", model_target.state_dict().keys())
    # exit(0)

    sys.stdout.flush()
    imagenet_path = "./resnet50-19c8e357.pth"
    checkpoint = torch.load(imagenet_path)
    model_budget.load_state_dict(checkpoint, strict= False)
    model_budget = model_budget.cuda(args.gpu)

    model_budget1.load_state_dict(checkpoint, strict= False)
    model_budget1 = model_budget1.cuda(args.gpu)


    total_epochs= 50

    save_dest = args.work_dir
    ...
    while ctr<1:
        ...
        for epoch in range(args.start_epoch, total_epochs):
            ...
            trainT_top1, trainT_top5, trainB_top1, trainB_top5, train_losses, train_speed, speed_data_loader, train_steps = train(train_loader, model_degrad, model_target, model_budget, optimizer_t,  
                                                                                                                 optimizer_b, train_criterion, train_entropy_criterion,train_smoothl1_criterion,  epoch + 1, display =  
        ...
        # # TARGET 评估 , 锁 anonymizer 参数， 只更新 target
        ...
        # sys.stdout.flush() # load pretrain
        ...
        # BUDGET 评估， 锁 anonymizer 参数， 只更新 BUDGET
